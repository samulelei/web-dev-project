<?php
  ...
  if($count == 1) {
     ...
     $sql5 = "SELECT bullet.date, bullet.text from bullet, estate, apartment, user WHERE bullet.estate_ID = estate.ID AND estate.ID = apartment.estate_ID AND apartment.user_ID = user.user_ID AND user.username = '$myusername' ORDER BY date DESC";
     ...
     $phone = $row4['phone'];
     $email = $row4['email'];
     $uusintiedote = utf8_encode($row5['text']);
     ...
     $_SESSION['uusintiedote'] = $uusintiedote;
