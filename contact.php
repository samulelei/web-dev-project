...
<?php
    if ($_SESSION['user'] != '' & $_SESSION['user'] != 'notlogged') {?>
    <form method="post" action="script/send_mail.php">
    ...
    <label for="email">Sähköposti <span class="tähti">*</span></label>
    <input type="text" id="email" name="email_address" value="<?php echo $_SESSION['email']; ?>" required>

    <label for="gsm">Puhelinnumero</label><span class="tähti" style="color: transparent">*</span>
...
<script>
function send() {
  alert("Viestisi on lähetetty! Paina ok palataksesi etusivulle");
}
</script>
</body>
</html>